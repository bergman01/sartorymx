...
return number_format($venta*$porciento/100 ,$decimales);
	...
	if($opcion=='UPD'){
		...
		if($opcion=='SAVE'){
			...
			echo $categoria = implode(',',$_POST["categoria"]);
...
informacion. Por favor Intenta de Nuevo.</div>
    ...
    <div class="table-responsive">
		...
		while($row=mysql_fetch_array($resultado)){
	     ...
	     <li id="li"><input type="checkbox" name="categoria[]" id="categoria"  value="<?php echo $row[0];?>" <? $busca = strpos($categoria, $row[0]); if ($busca==TRUE) { echo 'checked="checked"'; } ?>> <?php echo html_entity_decode($row[1],ENT_QUOTES);?></li>
