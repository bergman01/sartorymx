...
if($correo->Send()) {
 ...
 where a.estatus=1 and a.id_articulo=$ids and a.categoria = c.id_categoria";
  ...
  $queryc="select * from categorias where id_categoria = $categoria";



  $resultadoc=mysql_query($queryc,$link);
...
function agregar() {
	...
	campo = '<tr><td><label>CANTIDAD: <span id="span_clave"><input type="number" min="00" max="9999999" name="unidades[]" style="width: 45px !important" size="5"></label></span></label></td><td><label> COLOR: <span id="span_clave"><input type="text" name="color[]" width="10px" size="5"></span></label></td></tr>';
...
<a href="index.php">INICIO</a><spam style="color: #FE0000"> > </spam><?php if($categoria != 'Busqueda'){?><a href="articulos.php?id=<?php echo $categoria;?>"><?php echo html_entity_decode($catnom,ENT_QUOTES);?></a><?php }else{?> <a href="busqueda.php?buscar=<?php echo $nombre_producto;?>">Busqueda</a><?php } ?> <spam style="color: #FE0000"> > </spam> <?php echo $nombre_producto;?> </h3>
...
<section id="content" >
						...
						<div class="span2">
							...
							<div id="sidebar" class="sidebar-left">
								...
								<ul>
									...
									$query9="select id_categoria,categoria from categorias where id_categoria in (".$id_cate.");";
      ...
      $nombre_categoria=html_entity_decode($row9[1], ENT_QUOTES);
			...
			?>
										...
										<li><a href="articulos.php?id=<?php echo $id_categoria;?>&h=<?php echo $_GET['h'];?>&f=<?php echo $_GET['f'];?>&us=<?php echo $_GET['us'];?>"><?php echo $nombre_categoria;?></a></li>
								...
								<div class="span8">
									...
									<div class="main-data">
										...
										<?php if($precio_venta == 0){?>
											...
											<br/><h5><span style="color: #FE0000">Favor de Cotizar Precio</span></h5>
											...
											<br/><h5><span style="color: #FE0000" >$ <?php echo $precio_venta;?> + IVA</span></h5>
											<?php }else{ $mayoreod='0.'.$mayoreo;?>
											<br/><h5><span style="color: #FE0000" >$ <?php echo round(($precio_venta*$mayoreod),2);?> + IVA</span></h5>
										...
										}else{ ?><form  method="post" action="agrega_cotizacion2.php">
											...
											?>
															...
															<input type="hidden" value="<?php echo $codigo_cotizacion;?>" name="cc">
										...
										<td>
											...
											<label>CANTIDAD: <span id="span_clave"><input type="number"  step="0.01" min="00" max="9999999" name="unidades[]" style="width: 55px !important" size="5" >
                     ...
                     $query2="select * from personalizacion where idpersonalizacion in(".$persona.");";
								...
								while($row2 = mysql_fetch_array($resultado2)){
											...
											<label for="comment">COMENTARIOS:</label></td>
                                                    ...
                                                    <button class="btn btn-flat" type="submit" onclick="return confirmar('Articulo agregado al cotizador')"><i class="fa fa-cart-plus" aria-hidden="true"></i> Agregar al carrito</button>
