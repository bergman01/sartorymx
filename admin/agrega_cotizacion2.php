	...
	$rtranxx = mysql_query($quey1,$link);
		...
		if($usuario != ''){
			for ($i=0; $i<=count($unidades); $i++) {	
				...
				$total = round($total,2);
		$tranx[$i]="insert into cotizacion(id_articulo,usuario,fecha,estatus,unidad,color,nombre,codigo,precio_venta,persona,comentario,total,codigo_cotizacion) values($ids,'$usuario',CURDATE(),0,$unidades[$i],'$color[$i]','$nombre','$codigo','$precio','$tipo','$comentario','$total','$codigo_cotizacion');";					
