...
$query="Select * from cotizacion where usuario='$usuario' and fecha='$fecha' and hora='$hora' limit 1;";
$resultado=mysql_query($query, $link);

while($row=mysql_fetch_array($resultado)){
$destinatario=utf8_decode($row['destinatario']);
$empresa=utf8_decode($row['empresa2']);
$contacto_empresa=utf8_decode($row['contacto_empresa']);
$email_empresa=utf8_decode($row['email_empresa']);
$celular_empresa=utf8_decode($row['celular_empresa']);
$saludo=utf8_decode($row['saludo']);
$despedida=utf8_decode($row['despedida']);

$cotizo=utf8_decode($row['cotizado_por2']);

$puesto=utf8_decode($row['puesto']);

$telefono=utf8_decode($row['telefono']);
$email=utf8_decode($row['email']);
$condicion=utf8_decode($row['condiciones']);
$nota=utf8_decode($row['notas']);
            }
...
<table width="600" height="402" class="centrada" style="padding-left:25px;" >';
                            ...
                            $query="Select * from cotizacion where usuario='$usuario' and fecha='$fecha' and hora='$hora'";
