         ...
         <td style="background-color:  #ffffff !important;border: 1px solid #fff;"><input type="submit" class="btn btn-primary" name="guardar" id="guardar" value="Eliminar Selección" onclick="return confirmar('¿Está seguro que desea eliminar la selección?')"></td>
              ...
              <table class="table table-bordered table-hover table-striped tablesorter">
                ...
                <?php
							...
							$query="SELECT DISTINCT usuario,fecha,estatus,hora,empresa2,destinatario,contacto_empresa,empresa,cotizado_por,envio,codigo_cotizacion FROM cotizacion where estatus=5 order by fecha desc,hora desc;";
                  ...
                  <td><a href="detalle_venta.php?id=<?php echo $row['usuario'];?>&f=<?php echo $row['fecha'];?>&h=<?php echo $row['hora'];?>&cc=<?php echo $row['codigo_cotizacion'];?>"><?php echo $row['usuario'];?></a></td>
                  <td><?php echo $row['codigo_cotizacion'];?></td>
                    ...
                    <td><?php echo html_entity_decode($row['empresa2'], ENT_QUOTES) ?></td>
                    <td><?php echo html_entity_decode($row['destinatario'], ENT_QUOTES); ?></td>
                    </form>
