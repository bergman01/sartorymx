...
$user=$_SESSION['admin_user'];
	...
	if($opcion=='UPD'){
		...
		$query="SELECT categoria,imagen,estatus,filtro,portada FROM categorias where id_categoria='$idreg' limit 1";
		...
		if($opcion=='SAVE'){
			...
			if(!empty($_FILES['filefoto']['name'])){
				if(move_uploaded_file($_FILES['filefoto']['tmp_name'], $target_path)) {
					...
					if($idreg!=0){
						...
						//echo $foto;
						 ...
						 $tranx="update categorias set categoria='$titulo',imagen='$foto',estatus='$publicar',filtro='$filtro',portada=$portada where id_categoria=$idreg";
						...
						$tranx="insert into categorias (categoria,imagen,estatus,fecha_creacion,filtro,portada)  values('$titulo', '$foto',$publicar, CURDATE(),'$filtro',$portada)";
				...
				if($idreg!=0){
					...
					$tranx="update categorias set categoria='$titulo',estatus='$publicar',filtro='$filtro',portada=$portada where id_categoria=$idreg";
...
informacion. Por favor Intenta de Nuevo.</div>
    ...
    <div class="table-responsive">
	  ...
	  <input type="hidden" id="hiurl" name="hiurl" value="<? echo $foto ?>"  /></td>
        ...
        <tr>
        	<td><label>En portada</label></td>
        	<td><input class="checkbox" name="portada" id="portada" type="checkbox" <? if ($portada==1) { echo 'checked="checked"'; } ?>/></td>
        </tr>
        <tr>
