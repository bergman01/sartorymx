...
error_reporting(0);
  ...
  //include_once("lib/util.php");
	...
	if($opcion=='UPD'){
		...
		$query="SELECT nombre,imagen,estatus,url FROM banners where id_banner='$idreg' limit 1";
			...
			if(!empty($_FILES['filefoto']['name'])){
				if(move_uploaded_file($_FILES['filefoto']['tmp_name'], $target_path)) {
					...
					if($idreg!=0){
						...
						//echo $foto;
						 ...
						 $tranx="update banners set nombre='$titulo',imagen='$foto',estatus='$publicar',url='$url' where id_banner=$idreg";
						...
						$tranx="insert into banners (nombre,imagen,estatus,fecha_creacion,url)  values('$titulo', '$foto',$publicar, CURDATE(),'$url')";
				...
				if($idreg!=0){
					...
					//echo $foto=$_POST["hiurl"];
					 ...
					 $tranx="update banners set nombre='$titulo',estatus='$publicar',url='$url' where id_banner=$idreg";
...
informacion. Por favor Intenta de Nuevo.</div>
   ...
   <?php if($opcion=="UPD"){?>
    ...
    <div class="modal-footer">
                    ...
                    <a href="ffiltro_banners.php" onclick="return confirmar('¿Est&aacute; seguro que desea salir,no se guardara el registro?')"><button type="button" class="btn btn-default" data-dismiss="modal" >Cancelar</button></a>
    ...
    <div class="modal-footer">
                    ...
                    <a href="filtro_banners.php" onclick="return confirmar('¿Est&aacute; seguro que desea salir,no se guardara el registro?')"><button type="button" class="btn btn-default" data-dismiss="modal" >Cancelar</button></a>
